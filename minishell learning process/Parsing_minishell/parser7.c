...
void check_expansion(t_var *var, t_new *cmd)
{
	while (var->next)
	{
		// if (!ft_strcmp(cmd->token + 1, var->key)

		// var = var->next;
	}
}

void dollar_expansion(t_var *var, t_new *cmd)
{
	int i;
	int count;
	...
	while (cmd->next)
	{
		if(cmd->flag)
		{
			if (cmd->token[0] == '$' && cmd->token[1] != '$') //$$$hi$
			{
				check_expansion(cmd, var);
			}
		}
	}
}
